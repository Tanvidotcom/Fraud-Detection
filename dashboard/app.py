...
# @st.cache_data
# def load_data():
#     return pd.read_csv('dashboard/risk_scored.csv')
...
# @st.cache_resource
# def load_scaler():
#     with open('dashboard/scaler.pkl', 'rb') as f:
#         return pickle.load(f)
...
#     # Search box
#     search = st.text_input("🔎 Search by index (row number)")
#     if search:
#         try:
#             idx = int(search)
#             row = df.iloc[[idx]]
...
#     row_input = st.text_input("Enter row number (0 to " + str(len(df)-1) + ")")
...
#             # Show SHAP waterfall from saved image
#             if prob >= 0.75:
#                 img_path = 'charts/shap_waterfall_fraud.png'
...
#             elif prob >= 0.4:
#                 img_path = 'charts/shap_waterfall_borderline.png'
...
#             else:
#                 img_path = 'charts/shap_waterfall_legitimate.png'
...
#             st.error(f"Error: {e}. Please enter a valid row number.")

#     st.markdown("---")
#     st.subheader("📊 Global SHAP Feature Importance")
#     st.image('charts/shap_summary.png')
import streamlit as st
import pandas as pd
import numpy as np
import pickle
